...
- Rules mirrored at `.agents/rules/` (always-on) — keep in sync with this file
- Workflows at `.agents/workflows/` (e.g. `/phase` runs the master build prompt)
- Verify current `.agents/` path layout at setup time; it has changed between IDE versions
